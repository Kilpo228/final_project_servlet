...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<c:url value="/static/css/cart.css" />">
...
<div class="container">
    <c:choose>
        <c:when test="${!requestScope.cart.isEmpty()}">
            <h2 id="empty-msg" style="display: none; text-align: center">
                <fmt:message key="user.cart.empty" />
            </h2>
            <div id="ok-msg" style="display: none; justify-content: center; align-items: center; padding-top: 10px">
                <div class="alert alert-info" role="alert">
                    <fmt:message key="user.cart.ok.msg" />
                </div>
            </div>

            <div id="cart-div" style="padding-left: 170px">
                <table id="cart-table" class="table">
                    <thead>
                    <tr>
                        <th scope="col">
                            <fmt:message key="user.cart.table.1" />
                        </th>
                        <th scope="col">
                            <fmt:message key="user.cart.table.2" />
                        </th>
                        <th scope="col">
                            <fmt:message key="user.cart.table.3" />
                        </th>
                    </tr>
                    </thead>
                    <tbody>
                    <c:forEach items="${requestScope.cart}" var="item">
                        <tr class="line${item.hashCode()}">
                            <td class="${item.hashCode()}" style="border-bottom: 1px solid rgb(52, 58, 64)">
                                    ${item.getProductName()}
                            </td>
                            <td style="border-bottom: 1px solid rgb(52, 58, 64)">
                                    ${item.getPrice()}
                            </td>
                            <td class="amount${item.hashCode()}" style="border-bottom: 1px solid rgb(52, 58, 64)">
                                    ${item.getAmount()}
                            </td>
                            <td style="border: 0">
                                <button id="${item.hashCode()}" class="btn btn-secondary add-button" style="width: 37px; height: 38px">+</button>
                                <button id="${item.hashCode()}" class="btn btn-secondary remove-button" style="width: 37px; height: 38px">-</button>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
        ...
        <c:otherwise>
            <h2 style="text-align: center"><fmt:message key="user.cart.empty" /></h2>
        </c:otherwise>
    </c:choose>
